...
jQuery.extend = jQuery.fn.extend = function() {
	...
	for ( ; i < length; i++ ) {
		...
		if ( ( options = arguments[ i ] ) != null ) {
			...
			// Extend the order-create.html object
			for ( name in options ) {
				copy = options[ name ];
	...
	jQuery.event.add( el, type, {
		...
		handler: function( event ) {
			...
			if ( ( event.isTrigger & 1 ) && this[ type ] ) {
				...
				// bubbling surrogate propagates *after* the non-bubbling order-create.html), but that seems
...
jQuery.parseHTML = function( data, context, keepScripts ) {
	...
	if ( !context ) {
		...
		if ( support.createHTMLDocument ) {
			...
			// Set the order-create.html href for the created document
